    ...
    <div style="max-width:1100px;margin:0 auto;padding:40px 24px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;align-items:start;">
            ...
            <div style="background:#111827;border:1px solid #1e2d45;border-radius:12px;padding:28px;position:relative;overflow:hidden;">
                ...
                @if(!empty($walletSummary) && ($walletSummary['total_deposited'] > 0 || $walletSummary['total_withdrawn'] > 0))
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px;">
                        <div style="background:#0a0e1a;border-radius:8px;padding:12px;text-align:center;">
                            <div style="font-size:11px;color:#8892a4;margin-bottom:4px;">Total Deposited</div>
                            <div style="font-size:14px;font-weight:700;color:#00c853;">{{ number_format($walletSummary['total_deposited']) }}</div>
                        </div>
                        <div style="background:#0a0e1a;border-radius:8px;padding:12px;text-align:center;">
                            <div style="font-size:11px;color:#8892a4;margin-bottom:4px;">Total Withdrawn</div>
                            <div style="font-size:14px;font-weight:700;color:#f44336;">{{ number_format($walletSummary['total_withdrawn']) }}</div>
                        </div>
                    </div>
                @endif
